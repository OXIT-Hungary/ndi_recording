...
class Visualization:
    ...
    def draw_current_detection(self, players_in_bev, ax):
        if players_in_bev is not None:
            ax.scatter(players_in_bev[:, 0], players_in_bev[:, 1], 
                    color='red', marker='o', label='Detections')
            
    def draw_tracked_objects(self, active_tracks, ax):
        for track in active_tracks:
            x, y = track.kf.x[0], track.kf.x[1]
            ax.scatter(x, y, color='blue', marker='x')
            ax.text(x, y, f'ID: {track.track_id}', fontsize=8,
    ...
    def draw_velocity_vector(self, track, ax):
        ...
        vx = track.kf.x[2]
        vy = track.kf.x[3]
        ax.arrow(x, y, vx, vy, head_width=0.5, 
                head_length=1, fc='green', ec='green')
