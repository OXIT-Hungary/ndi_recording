...
class Track:
    def __init__(self, track_id, initial_pos):
        ...
        self.kf.x = np.array([initial_pos[0], initial_pos[1], 0., 0.])  # [x, y, vx, vy]
...
class Tracker:
    ...
    def update(self, detections):
        ...
        if len(self.tracks) > 0 and len(detections) > 0:
            cost = distance.cdist(
                [track.kf.x[:2] for track in self.tracks],
                detections,
                'euclidean'
            )
        ...
        self.tracks = [t for t in self.tracks 
                      ...
                      and (t.hits >= self.min_hits or t.age < 10)]
        
        gravity_center = self.get_cluster_centroid()

        # Return confirmed tracks
        return gravity_center, [t for t in self.tracks if t.hits >= self.min_hits]
    ...
    def get_cluster_centroid(self):
        positions = np.array([t.kf.x[:2] for t in self.tracks])
        
        if len(positions) < 2:
            return None, None, None

        # DBSCAN clustering
        db = DBSCAN(eps=self.eps, min_samples=self.min_samples).fit(positions)
        labels = db.labels_
